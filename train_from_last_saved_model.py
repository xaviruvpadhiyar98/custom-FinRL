...
model_ppo = PPO.load(
    path=r"trained_models\BEST_MODEL\ppo\best_model_best_hyperparams_245000_steps.zip",
    env=train_env,
...
def calculate_sharpe_and_get_ending_amount(infos: List[dict]):
    ...
    info = infos[0].values()
    current_holdings = np.array([i["current_holdings"] for i in info])
...
checkpoint_callback = CheckpointCallback(
    ...
    save_path=f"{TRAINED_MODEL_DIR}/BEST_MODEL/{MODEL_NAME}/retraining/",
    name_prefix="retrained_model",
